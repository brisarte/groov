...
ofPoint lookAt;
ofImage olho, orbita, girassol, logoBrisa;
...
void ofApp::update() {
	...
	if(time0 - ultimoEvento > tempoBrisa) {
		...
		/* Horror
		inicioFbo[2] = 0; // null
		numeroBrisaFbo[2] = 13; // video sereias
		*/
...
void ofApp::draw() {
	...
	logoBrisa.draw(vw*0.2 + intControl ,vh*0.6 + intControl2, logoBrisa.getWidth()*0.3, logoBrisa.getHeight()*0.3 );
...
//	imgGray.mirror(false,true);
	...
	for (int i = 0; i < numPixelsNoise; i++) {
		pixNoise[i] = ofClamp(ofMap(pixNoise[i], 0, 255, 20, 255), 0, 255); // Aumenta contraste de distancia
	}
...
void desenhaAlphaVideoRGB( ofVideoPlayer &vid) {
	...
	shaderVideoAlpha.begin();

	vid.draw( 0, 0, 1024, 768);

	shaderVideoAlpha.end();
